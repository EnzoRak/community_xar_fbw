...
function p.__get_tex()
    return "block_concrete_green_border"
end
...
function p.__main()
    ...
    for z = 0,15 do
        ...
        if( is_solid ) then
            if randi (1,1600) == 1 then
				set_pos(x,y,z, "XAR_MINI_AMMO_FOREST_AMMO") --Making it solid.
			elseif randi (1,6400) == 1 then
				set_pos(x,y,z, "block_xar_minigun_flower_fruit_3_treasure") --Treasure!
			else 
				set_pos(x,y,z, "block_xar_minigun_flower_fruit_3") --Making it solid.
			end
		elseif randi(1,5000) == 1 then
			add_bent(x,y,z, "bent_base_ring_green")
		elseif randi(1,50) == 1 then
			set_pos(x,y,z, "XAR_ASCEND_SIMPLE")
		elseif randi(1,800) == 1 then
			if randi(1,160) == 1 then
				add_ment(x,y,z, "ment_monster_beholder_lesser")
			end
        end
    end end end
end
