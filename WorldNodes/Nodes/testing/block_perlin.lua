...
function p.__main()
    ...
    local d1 = p.setup_xyz(0, 1, 0)
    --local d2 = p.setup(1, 1, 0)
    --local d3 = p.setup(2, 2, 0)
    for x = 0,15 do for y = 0,15 do for z = 0,15 do
        local t1 = p.transform_xyz(d1,x,y,z)
        --local t2 = p.transform(d2,x,y,z)
        --local t3 = p.transform(d3,x,y,z)
        local v = perlin_noise_xyz(0, t1.x,t1.y,t1.z)--*0.5 + perlin_noise_xyz(1, t2.x,t2.y,t2.z) + perlin_noise_xyz(2, t3.x,t3.y,t3.z)*2
        if v > 0 then
            set_pos(x, y, z, "s")
        end
    end end end
